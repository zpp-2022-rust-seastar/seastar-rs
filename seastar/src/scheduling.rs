...
mod ffi {
    unsafe extern "C++" {
        ...
        fn sg_set_shares(sg: &UniquePtr<scheduling_group>, shares: f32);

        fn sg_equal(sg1: &scheduling_group, sg2: &scheduling_group) -> bool;

        fn create_sg(sg: &mut UniquePtr<scheduling_group>, name: &str, shares: f32) -> VoidFuture;

        fn destroy_sg(sg: &UniquePtr<scheduling_group>) -> VoidFuture;

        fn rename_sg(sg: &UniquePtr<scheduling_group>, new_name: &str) -> VoidFuture;

        fn max_sg() -> u32;

        fn current_sg() -> UniquePtr<scheduling_group>;
    }
}
...
impl SchedulingGroup {
    ...
    /// [`create`](SchedulingGroup::create). The destroyed group must not be currently
    /// in use and must not be used later.
    ///
