...
void sg_set_shares(const std::unique_ptr<scheduling_group>& sg, float shares) {
...
VoidFuture create_sg(std::unique_ptr<scheduling_group>& sg, rust::str name, float shares) {
    ...
    sg = std::make_unique<scheduling_group>(std::move(new_sg));
}

VoidFuture destroy_sg(const std::unique_ptr<scheduling_group>& sg) {
    co_await seastar::destroy_scheduling_group(*sg);
}

VoidFuture rename_sg(const std::unique_ptr<scheduling_group>& sg, rust::str new_name) {
...
std::unique_ptr<scheduling_group> current_sg() {
    scheduling_group sg = seastar::current_scheduling_group();
    return std::make_unique<scheduling_group>(std::move(sg));
}
